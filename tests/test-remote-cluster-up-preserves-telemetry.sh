...
' "${CAPTURE_FILE}" >/dev/null || {
  echo "[FAIL] remote cluster-up did not preserve resolved cluster configuration" >&2
  cat "${CAPTURE_FILE}" >&2
  exit 1
}

echo "[PASS] remote cluster-up preserves resolved telemetry and source settings"
